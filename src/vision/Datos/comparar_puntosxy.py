...
# Proyectar las coordenadas en el plano XY
predicciones_xy = predicciones[:, :2]
valores_reales_xy = valores_reales[:, :2]

# Crear la figura y los subplots
fig, axs = plt.subplots(ncols=2, figsize=(10,5))

# Graficar las predicciones2 en el primer subplot
axs[0].scatter(predicciones_xy[:, 0], predicciones_xy[:, 1])
axs[0].set_xlabel('X')
axs[0].set_ylabel('Y')
axs[0].set_title('Predicciones')

# Graficar los valores reales en el segundo subplot
axs[1].scatter(valores_reales_xy[:, 0], valores_reales_xy[:, 1])
axs[1].set_xlabel('X')
axs[1].set_ylabel('Y')
